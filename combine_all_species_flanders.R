...
write.table(species, "species_habitats_hot_cold_flanders.csv", sep = ";", row.names = FALSE, col.names = TRUE)
...
#moth <- moth[moth$Stratum_omschrijving %in% "Flanders",]
...
dragonflies <- dragonflies[dragonflies$Stratum_omschrijving %in% "Flanders",]
...
mammals <- mammals[mammals$Stratum_omschrijving %in% "Flanders",]
...
amphibians <- amphibians[amphibians$Stratum_omschrijving %in% "Flanders",]
...
grasshoppers <- grasshoppers[grasshoppers$Stratum_omschrijving %in% "Flanders",]
...
reptiles <- reptiles[reptiles$Stratum_omschrijving %in% "Flanders",]
...
butterflies <- butterflies[butterflies$Stratum_omschrijving %in% "Flanders",]
summary(butterflies)
...
bats <- bats[bats$region %in% "Flanders",]
bats$group_tax <- "ZOG"
...
birds <- read.table("all_Indices_All_Trends_fla.csv", sep = ";", header = TRUE)
...
birds_trend <- birds_trend[! birds_trend$code %in% c(5120,16631,1710,11980,1560,1619,120,20800,950,1440,1590,1750,1960,5750,6150,6240,8460,10202,11370,12260,12650,13640,14900,16380,16634,18820,30040,70,90,100,720,980,1190,1220, 1310, 1340,1520, 1570, 1610, 1620, 1660, 1670, 1700, 1730, 1770,1780, 1790, 1820, 1840, 1861, 1910, 1940, 1980, 2020, 2030, 2200, 2230, 2310,2380,2390,2600,2610,2630,2670,3010,3100,3200,3260,3320,4070,4210,4330,4560,4690,4700,4850,4970,5010,5170,5190,5290,5410,5450,5480,5530,5560,5820,5900,5910,5920,6270, 6650,7120, 7350,7570,7610,7670,7680,7780,8310,8550,9810,10010,10172,11370,11460,11860,12010,12380,12530,13070,15630,16380,16630,18580,30010),]
birds$group_tax <- "BIR"
birds <- birds[, c(2,27,5,15:26,6,7,8)]
colnames(birds) <- c("code", "group_tax", "type", "2007", "2008", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018","n_square","slope","se_slope")
birds_trend <- birds_trend[! birds_trend$code %in% c(5120,16631,1710,11980,1560,1619,120,20800,950,1440,1590,1750,1960,5750,6150,6240,8460,10202,11370,12260,12650,13640,14900,16380,16634,18820,30040,70,90,100,720,980,1190,1220, 1310, 1340,1520, 1570, 1610, 1620, 1660, 1670, 1700, 1730, 1770,1780, 1790, 1820, 1840, 1861, 1910, 1940, 1980, 2020, 2030, 2200, 2230, 2310,2380,2390,2600,2610,2630,2670,3010,3100,3200,3260,3320,4070,4210,4330,4560,4690,4700,4850,4970,5010,5170,5190,5290,5410,5450,5480,5530, 5560,5820,5900,5910,5920,6270, 6650,7120, 7350,7570,7610,7670,7680,7780,8310,8550,9810,10010,10172,11370,11460,11860,12010,12380,12530,13070,15630,16380,16630,18580,30010),]
slope_square_bi <- birds[,c(1,2,3,16,17,18)]
...
heterocera <- read.table("Moths_Flanders_TRIM.csv", sep = ";", header = TRUE)
heterocera <- heterocera[heterocera$Recordtype_name %in% c("indices", "se_indices"),]
heterocera_trend <- heterocera[! heterocera$Slope_imputed_classification %in% "",]
heterocera_trend <- heterocera_trend[,c("Species_number","Slope_imputed_classification","soort")]
heterocera_trend <- heterocera_trend[,c(1,3,2)]
colnames(heterocera_trend) <- c("code", "tax", "Multipl_trend_category")
heterocera_trend$code <- heterocera_trend$code*100000
heterocera$group_tax <- "HET"
heterocera <- heterocera[, c(2,26,25,5,15:24, 6,7,8)]
colnames(heterocera) <- c("code", "group_tax", "tax", "type", "2009", "2010", "2011", "2012", "2013", "2014", "2015", "2016", "2017", "2018","n_square", "slope", "se_slope")
slope_square_he <- heterocera[,c(1,2,3,4,15,16,17)]
slope_square_he <- slope_square_he[slope_square_he$type %in% "indices",]
slope_square_he <- slope_square_he[,c(1,2,3,4,5,6,7)]
slope_square_he$code <- slope_square_he$code*100000
heterocera <- melt(heterocera, id.vars = c("code", "group_tax", "tax", "type","n_square", "slope", "se_slope"))
heterocera$code <- 100000*heterocera$code
heterocera_se <- heterocera[heterocera$type %in% "se_indices", ]
heterocera_se <- heterocera_se[,c("code", "variable", "value")]
colnames(heterocera_se) <- c("code", "year", "se")
heterocera_index <- heterocera[heterocera$type %in% "indices", ]
heterocera_index <- heterocera_index[, c("code", "group_tax", "tax",  "variable", "value","n_square", "slope", "se_slope")]
colnames(heterocera_index) <- c("code", "group_tax", "tax", "year", "index","n_square", "slope", "se_slope")
heterocera <- merge(heterocera_index, heterocera_se, by.x = c("code", "year"), by.y = c("code", "year"), all = TRUE )
heterocera <- heterocera[,c("code", "year", "group_tax", "tax", "index", "se")]
colnames(heterocera) <- c("code", "year", "group_tax", "tax", "index", "se")
mode(heterocera$se)
heterocera$se <- chartr(old = ",", new = ".", x = heterocera$se)
heterocera$se <- as.numeric(heterocera$se)
heterocera$index <- chartr(old = ",", new = ".", x = heterocera$index)
heterocera$index <- as.numeric(heterocera$index)

#############################################################################################

all_species <- rbind(birds, mammals, dragonflies, butterflies, grasshoppers, amphibians, reptiles, heterocera)
...
all_species_trend <- rbind(birds_trend, mammals_trend, dragonflies_trend, reptiles_trend, amphibians_trend, butterflies_trend, grasshoppers_trend, heterocera_trend) 

all_species <- merge(all_species, all_species_trend, by.x = c("code", "tax"), by.y = c("code", "tax"), all.x = TRUE)
...
all_species <- all_species[! all_species$code %in% c("1700", "1660", "18820", "7120", "9810","15630"),]
all_species <- all_species[! all_species$tax %in% "Cygnus atrata",]
...
min_year <- as.data.frame(tapply(all_species$year, all_species$code,  min))
min_year
max_year <- tapply(all_species$year, all_species$code,  max)
...
slopes_squares <- rbind(slope_square_am,slope_square_bi, slope_square_bu, slope_square_dr, slope_square_gr, slope_square_ma, slope_square_re, slope_square_he)
...
write.table(synthese, "synthese_species_LPI_flanders.csv", sep = ";", dec = ",", col.names = TRUE , row.names = FALSE)


##########################################################################################################

species_list <- as.data.frame(unique(all_species[,c("code", "tax", "group_tax", "Habitat", "category", "STI..oC.")]))
write.table(species_list, "LPI_species_habitat_flanders.csv", sep = ";", dec = ",", col.names = TRUE, row.names = FALSE)
...
for (i in 1 : length(liste_belgium)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\GRAPHS\\FLANDERS")
  
  
  ggsave (paste0("graphe_", liste_belgium[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_belgium[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_belgium[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_all_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS")

write.table (ALL, file = "ALL_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI.R")
...
for (i in 1 : length(liste_agriculture)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\AGRICULTURE")
  
  
  ggsave (paste0("graphe_", liste_agriculture[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_agriculture[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_agriculture[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_agriculture_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (AGRIC, file = "AGRIC_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_agriculture.R")
...
for (i in 1 : length(liste_grasslands)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\GRASSLANDS")
...
source("LPI_Flanders_MSI_grasslands.R")
...
for (i in 1 : length(liste_hot)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\HOT")
  
  
  ggsave (paste0("graphe_", liste_hot[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_hot[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_hot[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_hot_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (HOT, file = "HOT_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_hot.R")
...
for (i in 1 : length(liste_cold)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\COLD")
  
  
  ggsave (paste0("graphe_", liste_cold[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_cold[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_cold[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_cold_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (COLD, file = "COLD_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_cold.R")
...
for (i in 1 : length(liste_open_natural)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\OPEN_NATURAL")
  
  
  ggsave (paste0("graphe_", liste_open_natural[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_open_natural[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_open_natural[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_open_natural_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (OPEN, file = "OPEN_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_open_natural.R")
...
for (i in 1 : length(liste_woodlands)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\WOODLANDS")
  
  
  ggsave (paste0("graphe_", liste_woodlands[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_woodlands[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_woodlands[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_woodlandsflanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (WOODLANDS, file = "WOODLANDS_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_woodlands.R")
...
for (i in 1 : length(liste_wetlands)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\WETLANDS")
  
  
  ggsave (paste0("graphe_", liste_wetlands[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_wetlands[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_wetlands[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_wetlands_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (WETLANDS, file = "WETLANDS_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_wetlands.R")
...
for (i in 1 : length(liste_urban)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\URBAN")
  
  
  ggsave (paste0("graphe_", liste_urban[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_urban[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_urban[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_urban_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (URBAN, file = "URBAN_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_urban.R")
...
for (i in 1 : length(liste_eurytopique)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\EURYTOPIQUE")
  
  
  ggsave (paste0("graphe_", liste_eurytopique[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_eurytopique[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_eurytopique[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_eurytopique_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (EURYTOPIQUE, file = "EURYTOPIQUE_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_eurytopique.R")
...
for (i in 1 : length(liste_birds)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\BIRDS")
  
  
  ggsave (paste0("graphe_", liste_birds[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_birds[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_birds[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_birds_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (BIRDS, file = "BIRDS_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_birds.R")
...
for (i in 1 : length(liste_amphibians)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\AMPHIBIANS")
  
  
  ggsave (paste0("graphe_", liste_amphibians[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_amphibians[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_amphibians[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_amphibians_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (AMPHIBIANS, file = "AMPHIBIANS_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_amphibians.R")
...
#for (i in 1 : length(liste_moths)) {
 ...
 # species <- moths_species[moths_species$code == liste_moths[i],]
  ...
  #setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\MOTHS")
  
  
#  ggsave (paste0("graphe_", liste_moths[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
#  ggsave (paste0("graphe_", liste_moths[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
#  write.table(species, (paste0("index_", liste_moths[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
#write.table (LL, file = "Indices_moths_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
#write.table (MOTHS, file = "MOTHS_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
...
for (i in 1 : length(liste_reptiles)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\REPTILES")
  
  
  ggsave (paste0("graphe_", liste_reptiles[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_reptiles[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_reptiles[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_reptiles_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (REPTILES, file = "REPTILES_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_reptiles.R")
...
for (i in 1 : length(liste_mammals)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\MAMMALS")
  
  
  ggsave (paste0("graphe_", liste_mammals[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_mammals[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_mammals[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_mammals_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (MAMMALS, file = "MAMMALS_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_mammals.R")
...
for (i in 1 : length(liste_dragonflies)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\DRAGONFLIES")
  
  
  ggsave (paste0("graphe_", liste_dragonflies[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_dragonflies[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_dragonflies[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_dragonflies_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (DRAGONFLIES, file = "DRAGONFLIES_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_dragonflies.R")
...
for (i in 1 : length(liste_grasshoppers)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\GRASSHOPPERS")
  
  
  ggsave (paste0("graphe_", liste_grasshoppers[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_grasshoppers[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_grasshoppers[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_grasshoppers_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (GRASSHOPPERS, file = "GRASSHOPPERS_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_grasshoppers.R")
...
for (i in 1 : length(liste_butterflies)) {
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\BUTTERFLIES")
  
  
  ggsave (paste0("graphe_", liste_butterflies[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_butterflies[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_butterflies[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_butterflies_flanders.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (BUTTERFLIES, file = "BUTTERFLIES_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_butterflies.R")
...
heterocera_species <- all_species[all_species$group_tax %in% "HET",]

liste_heterocera <- as.list(unique(heterocera_species$code))
head(liste_heterocera)

mingraph <- min(heterocera_species$year)
maxgraph <- max(heterocera_species$year)


for (i in 1 : length(liste_heterocera)) {
  
  species <- heterocera_species[heterocera_species$code == liste_heterocera[i],]
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\HETEROCERA")
  
  
  ggsave (paste0("graphe_", liste_heterocera[i],"_trends_flanders.pdf"), plot = XYZ, width=20/2.54, height = 28/2.54)
  ggsave (paste0("graphe_", liste_heterocera[i],"_trends_flanders.png"), plot = XYZ, width=20/2.54, height = 28/2.54)
  write.table(species, (paste0("index_", liste_heterocera[i],"_flanders.txt")), sep = "\t", col.names = TRUE, row.names = FALSE)
...
write.table (LL, file = "Indices_heterocera_flanders.csv", row.names = FALSE, sep = ";", dec = ",")


HETEROCERA <- LL[,c("euring", "imputed", "se_imp", "year")]
HETEROCERA$index <- HETEROCERA[,c("imputed")]
HETEROCERA$se <- HETEROCERA[,c("se_imp")]
#HETEROCERA$taxprio <- as.character(HETEROCERA$taxprio)
#HETEROCERA[HETEROCERA$taxprio == "Emberiza calandra" & !is.na(HETEROCERA$taxprio), "taxprio"] <- "Miliaria calandra"
#HETEROCERA <- merge(HETEROCERA, g, by.x = "euring", by.y="code_euring")
#unique(HETEROCERA$nom_latin)
HETEROCERA <- HETEROCERA[,c("euring", "year", "index", "se")]
colnames(HETEROCERA) <- c("species", "year", "index", "se")
head(HETEROCERA)
summary(HETEROCERA)

setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\HETEROCERA")

write.table (HETEROCERA, file = "HETEROCERA_MSI_FLANDERS.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_Heterocera.R")
...
all_species1 <- rbind(birds, mammals, dragonflies, butterflies, grasshoppers, amphibians, reptiles)
all_species1$se <- as.numeric(all_species1$se)
all_species1$index <- as.numeric((all_species1$index))
all_species1$ET <- all_species1$se*1.96
all_species1$ICpos <- all_species1$index+all_species1$ET
all_species1$ICneg <- all_species1$index-all_species1$ET
all_species1$year <- as.numeric(as.character(all_species1$year))

summary(all_species1)

all_species1_trend <- rbind(birds_trend, mammals_trend, dragonflies_trend, reptiles_trend, amphibians_trend, butterflies_trend, grasshoppers_trend, birds_trend)
all_species1 <- merge(all_species1, all_species1_trend, by.x = c("code", "tax"), by.y = c("code", "tax"), all.x = TRUE)
all_species1 <- merge(all_species1, species, by.x = "tax", by.y = "Species", all.x = TRUE)
#all_species1 <- all_species1[,c(1:11)]
#all_species1 <- merge(all_species1, hot_cold, by.x = "tax", by.y = "Species", all.x = TRUE)
#all_species1 <- all_species1[,c(1:11,14)]

mode(all_species1$tax)
class(all_species1$tax)

all_species1[all_species1$tax %in% "Miliaria calandra", "Habitat"] <- "agriculture"
all_species1[all_species1$tax %in% "Sylvia communis", "Habitat"] <- "agriculture"
all_species1[all_species1$tax %in% "Sturnus vulgaris", "Habitat"] <- "agriculture"
all_species1[all_species1$tax %in% "Saxicola torquatus", "Habitat"] <- "agriculture"
all_species1[all_species1$tax %in% "Streptopelia turtur", "Habitat"] <- "agriculture"
all_species1[all_species1$code %in% "389", "Habitat"] <- "woodlands" # squirrel
all_species1[all_species1$group_tax %in% "LIB", "Habitat"] <- "wetlands" # all dragonflies in wetlands habitats
all_species1[all_species1$Habitat %in% NA, "Habitat"] <- "eurytopique"

all_species1$tax <- as.character(all_species1$tax)

#bug <- all_species1[all_species1$group_tax %in% "VLI" & all_species1$group %in% "Cold" & all_species1$Habitat %in% "open nature land",]
#bug$tax <- as.factor(bug$tax)
#summary(bug)
...
#all_species1 <- all_species1[! all_species1$tax %in% exit] ## warning ! to remove when problematic butterflies are found.

summary(all_species1)

all_species1 <- all_species1[! all_species1$tax %in% out,] #change tax fro factor to character
all_species1 <- all_species1[! all_species1$tax %in% " Hemianax ephippiger Burmeister, 1839",]
all_species1 <- all_species1[! all_species1$code == "401",]
all_species1 <- all_species1[! all_species1$code == "5772714",]
all_species1 <- all_species1[! all_species1$code == c("1700", "1660", "18820", "7120", "9810","15630"),]
all_species1 <- all_species1[! all_species1$tax %in% "Cygnus atrata",]

summary(all_species1)

all_species1$tax <- as.factor(all_species1$tax)

summary(all_species1)

all_species1[all_species1$index %in% NA, "index"] <- 0
all_species1[all_species1$se %in% NA, "se"] <- 0
all_species1[all_species1$ET %in% NA, "ET"] <- 0
all_species1[all_species1$ICpos %in% NA, "ICpos"] <- 0
all_species1[all_species1$ICneg %in% NA, "ICneg"] <- 0
all_species1 <- all_species1[! (all_species1$group_tax %in% "BIR" & all_species1$Multipl_trend_category %in% NA),]
#all_species1 <- all_species1[! all_species1$Multipl_trend_category %in% "Uncertain",]

summary(all_species1)

##########################################################################################################

min_year <- as.data.frame(tapply(all_species1$year, all_species1$tax,  min))
min_year <- min_year[! min_year$`tapply(all_species1$year, all_species1$tax, min)` %in% NA,]
min_year
max_year <- as.data.frame(tapply(all_species1$year, all_species1$tax,  max))
max_year <- max_year[! max_year$`tapply(all_species1$year, all_species1$tax, max)` %in% NA,]
max_year

synthese1 <- as.data.frame(unique(all_species1[,c("code", "tax", "group_tax", "Multipl_trend_category", "Habitat", "group", "category")]))
synthese1 <- synthese1[order(synthese$tax),]
synthese1 <- cbind(synthese1, min_year)
synthese1 <- cbind(synthese1, max_year)
colnames(synthese1) <- c("code", "tax", "group_tax", "Multipl_trend_category", "Habitat", "group", "category", "first_year", "last_year")
synthese1 <- synthese1[,c("code", "tax", "group_tax", "Multipl_trend_category","first_year", "last_year", "Habitat", "group", "category")]

write.table(synthese1, "synthese_species_LPI_flanders_without_moth.csv", sep = ";", col.names = TRUE , row.names = FALSE)


##########################################################################################################

species_list <- as.data.frame(unique(all_species1[,c("code", "tax", "group_tax", "Habitat", "group", "STI..oC.", "median_STI", "category")]))
write.table(species_list, "LPI_species_habitat_flanders_without_moth.csv", sep = ";", dec = ",", col.names = TRUE, row.names = FALSE)


liste_belgium <- as.list(unique(all_species1$code))
head(liste_belgium)

mingraph <- min(all_species1$year)
maxgraph <- max(all_species1$year)


for (i in 1 : length(liste_belgium)) {
  
  species <- all_species1[all_species1$code == liste_belgium[i],]
  ...
  setwd("D:\\COA\\CBM_BELGIUM\\For_LPIsCalcul\\MSI\\FLANDERS\\LPI_WITHOUT_MOTH")
...
colnames(LL) <- c("euring","taxprio","year", "group_tax", "imputed", "se_imp" ,"ET", "ICpos", "ICneg", "trend", "habitat")

LL <- unique(LL)
LL

write.table (LL, file = "Indices_all_flanders_without_moth.csv", row.names = FALSE, sep = ";", dec = ",")
...
write.table (ALL, file = "ALL_MSI_FLANDERS_WITHOUT_MOTH.txt", row.names = FALSE, sep = "\t")
source("LPI_Flanders_MSI_without_moth.R")
